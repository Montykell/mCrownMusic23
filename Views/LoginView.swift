...
struct LoginView: View {
    ...
    @State private var navigateToReactivate: Bool = false // ✅ Reactivate navigation
                        ...
                        VStack(spacing: 20) {
                            ...
                            VStack(spacing: 16) {
                                ...
                                Button(action: {
                                    ...
                                    Task {
                                        let success = await loginViewModel.authenticateWithBiometrics()
                                        await MainActor.run {
                                            isLoading = false
                                            if !success {
                                ...
                                // Reactivate Account Navigation
                                NavigationLink(destination: ReactivateAccountView().environmentObject(loginViewModel),
                                               isActive: $navigateToReactivate) {
    ...
    private func handleLogin() {
        ...
        Task {
            let success = await loginViewModel.loginAsync()
            await MainActor.run {
                isLoading = false
            }
            ...
            if success {
                ...
                db.collection("users").whereField("email", isEqualTo: email).getDocuments { snapshot, error in
                    if let error = error {
                        passwordError = "Error checking account status: \(error.localizedDescription)"
                        return
                    }

                    guard let document = snapshot?.documents.first else {
                        passwordError = "No account found."
                        return
                    }
                    ...
                    if !isActive {
                        // Redirect to ReactivateAccountView
                        navigateToReactivate = true
                    } else {
                        // Normal login flow
