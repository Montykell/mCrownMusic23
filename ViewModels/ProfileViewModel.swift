...
class ProfileViewModel: ObservableObject {
    ...
    func deletePost(postId: String, completion: @escaping (Bool) -> Void) {
        let rtdbRef = rtdb.child("updates").child(postId)
        let firestoreRef = db.collection("updates").document(postId)
        
        rtdbRef.removeValue { rtdbError, _ in
            if let rtdbError = rtdbError {
                print("Error deleting post from RTDB: \(rtdbError)")
                completion(false)
                return
            }
            
            firestoreRef.delete { firestoreError in
                if let firestoreError = firestoreError {
                    print("Error deleting post from Firestore: \(firestoreError)")
                    completion(false)
                    return
                }
                
                print("Post deleted from both RTDB and Firestore successfully.")
                self.fetchUserPosts()
                completion(true)
            }
        }
